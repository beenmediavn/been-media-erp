...
export default function JobPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const ass=(d.assignments||[]).map((a:any)=>{
        ...
        return { employee_id:emp?.id||"", role:a.role||"Thợ chụp", salary_amount:Number(a.salary_amount||((a.role||"").includes("quay")?900000:700000)), note:a.note||"", client_requested:Boolean(a.client_requested) };
      });
  ...
  const addReservedWorker = (dayIndex: number, locationIndex: number, reserve: any) => {
    ...
    setDays((prev) => prev.map((day, i) => {
      ...
      return {...day, locations: day.locations.map((loc:any,j:number)=>j===locationIndex?{...loc, assignments:[...loc.assignments, {employee_id: reserve.employee_id, role: reserve.role || emp?.role || "Thợ chụp", salary_amount: Number(emp?.base_fee || (String(reserve.role).includes("quay") ? 900000 : 700000)), note: "Thêm từ thợ dự phòng", client_requested: false}]}:loc)};
    }));
  };
                  ...
                  <div className="space-y-2">
                    {location.assignments.map((assignment: any, assignmentIndex: number) => (
                      <div key={assignmentIndex} className={`rounded-xl border p-3 ${assignment.client_requested?'border-red-300 bg-red-50':'bg-slate-50'}`}>
                        ...
                        <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm font-semibold text-red-700"><input type="checkbox" checked={Boolean(assignment.client_requested)} onChange={e=>updateAssignment(dayIndex, locationIndex, assignmentIndex, "client_requested", e.target.checked)} /> ⭐ Khách chỉ định đích danh thợ này</label>
