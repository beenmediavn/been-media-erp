...
-- ERP V2: phân quyền đăng nhập nhân sự
alter table employees add column if not exists app_role text default 'viewer';
alter table employees add column if not exists can_login boolean default true;
...
set app_role = case
  ...
  else coalesce(app_role, 'viewer')
end
where app_role is null or app_role = 'viewer';

insert into employees (full_name, phone, role, app_role, username, password, base_fee, active, can_login)
select 'Nguyễn Anh Tuấn', '0585557555', 'Admin', 'admin', 'admin', '123456', 0, true, true
where not exists (select 1 from employees where username = 'admin');

-- BEEN MEDIA ERP V3: Sổ thu chi tổng hợp
